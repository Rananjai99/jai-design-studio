"use client";
/** Row 7 — Solid colour cells. 16 cols landing / 48 cols home. No border. */
import React from "react";
import { motion } from "framer-motion";
import { CanvasPage, CANVAS } from "./WorkingCanvas";
import { useTheme } from "@/context/ThemeContext";
import styles from "./CanvasRows.module.css";

export function CanvasRow7({ page }: { page: CanvasPage }) {
  const { theme } = useTheme();
  const cols = page === "landing" ? CANVAS.ROW7_COLS_LANDING : CANVAS.ROW7_COLS_HOME;
  return (
    <div className={`${styles.row} ${styles.row7} ${styles[page]}`}>
      {Array.from({ length: cols }).map((_, i) => (
        <motion.div
          key={i} className={styles.solidCell}
          style={{ backgroundColor: theme.row7[i] ?? "#fff0cc" }}
          animate={{ backgroundColor: theme.row7[i] ?? "#fff0cc" }}
          transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1], delay: i * 0.005 }}
        />
      ))}
    </div>
  );
}
